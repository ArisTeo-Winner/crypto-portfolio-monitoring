...
@Component
public class PasswordResetRateLimiter extends AbstractRequestRateLimiter {
  ...
  public PasswordResetRateLimiter(
      ...
      @Value("${security.password-reset-rate-limit.enabled:true}") boolean enabled,
      @Value("${security.password-reset-rate-limit.max-attempts:3}") int maxAttempts,
      @Value("${security.password-reset-rate-limit.window-seconds:900}") long windowSeconds,
      @Value("${security.password-reset-rate-limit.cleanup-interval-seconds:600}")
  ...
  protected RuntimeException createRateLimitException(long retryAfterSeconds) {
    return new TooManyPasswordResetRequestsException(retryAfterSeconds);
  }
}
