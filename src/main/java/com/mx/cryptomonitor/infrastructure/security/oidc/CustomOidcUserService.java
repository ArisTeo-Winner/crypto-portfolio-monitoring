...
import com.mx.cryptomonitor.domain.repositories.UserAuthProviderRepository;
...
public class CustomOidcUserService extends OidcUserService{
	...
	public OidcUser loadUser(OidcUserRequest userRequest) {
		...
		if (sub == null) throw new IllegalStateException("OIDC sub nulo");
		
		Optional<UserAuthProvider> existingLink = userAuthProviderRepository
				.findByAuthProviderAndProviderId(AuthProvider.GOOGLE, sub);
		User user;
		...
		if (existingLink.isPresent()) {
			...
			Optional<User> byEmail = (email != null) ? userRepository.findByEmailIgnoreCase(email) : Optional.empty();
			...
			Role r = roleRepository.findByName(DEFAULT_ROLE)
			...
			UserAuthProvider uap = UserAuthProvider.builder()
					.user(user)
		...
		DefaultOidcUser delegate = new DefaultOidcUser(authorities, oidc.getIdToken(), oidc.getUserInfo() );
		
		return new DomainOidcUser(delegate, user);
	}
