...
public class FmpCatalogAdapter implements CatalogFetchPort {
  ...
  public List<AssetCatalogDto> fetchTopStocks(int limit) {
    try {
      ...
      // Nunca loguear e.getMessage() de WebClient: puede contener la URL con ?apikey=...
      log.warn(
          "FMP error transitorio en fetchTopStocks, conservando catalogo previo: {}", detailOf(e));
      return List.of();
    }
  }
  ...
  public List<AssetCatalogDto> fetchTopEtfs(int limit) {
    try {
      ...
      // Nunca loguear e.getMessage() de WebClient: puede contener la URL con ?apikey=...
      log.warn(
          "FMP error transitorio en fetchTopEtfs, conservando catalogo previo: {}", detailOf(e));
      return List.of();
    }
  }

  private static String detailOf(Exception e) {
    return (e instanceof WebClientResponseException w)
