...
public class CustomOAuth2UserService extends DefaultOAuth2UserService{
	...
	public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
		...
		String givenName = asString(attr.get("givin_name"));
		...
		if (sub == null || email == null) {
			...
			throw new OAuth2AuthenticationException("Falta claims obligatorios (sub/email) en respuesta de Google");
		}
		
		Optional<UserAuthProvider> linkOpt = userAuthProviderRepository
				.findByAuthProviderAndProviderId(AuthProvider.GOOGLE, pictureUrl);
		...
		if (linkOpt.isPresent()) {
			...
			if (user == null) {
				...
				if(!userAuthProviderRepository.existsByUserIdAndAuthProvider(user.getId(), AuthProvider.GOOGLE)) {
					userAuthProviderRepository.save(UserAuthProvider.builder()
							.user(user)
							.authProvider(AuthProvider.GOOGLE)
							.providerId(sub)
